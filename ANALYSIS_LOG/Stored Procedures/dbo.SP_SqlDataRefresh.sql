...
-- =============================================
CREATE PROCEDURE[dbo].[SP_SqlDataRefresh]
--(@DNAME VARCHAR(1024))
AS
BEGIN
	...
	--SET @DBNAME=@DNAME
	DELETE FROM T_BI_LOG_DW2_SQLLOG_STAGING

	

	SELECT @LASTREFRESHDATA=[LastRunDateTime] FROM [ANALYSIS_LOG].[dbo].[M_BI_LOG_DW2_EXECUTION] WHERE [PackageName]='SP_SQLDataRefresh'

	INSERT INTO T_BI_LOG_DW2_SQLLOG_STAGING   SELECT [Start Date],[Database Name],[Query Details],Email,[Application Name],[Login Type]   FROM VUSR_T_SQL_DETAILS
	...
	BEGIN
	   INSERT INTO T_BI_LOG_DW2_SQLLOG 
		SELECT @FIELDNAME AS FILEDNAME, [Start Date],[Database Name],count(*),Email,[Application Name],[Login Type],@TBNAME FROM  T_BI_LOG_DW2_SQLLOG_STAGING  --VUSR_T_SQL_DETAILS
	    ...
	    WHERE (([Query Details] like '%.[[]' + @FIELDNAME + ']%' OR [Query Details] like '%["]' + @FIELDNAME + '"%' OR [Query Details] like '%.' + @FIELDNAME + '%' 
	...
	BEGIN
	   INSERT INTO T_BI_LOG_DW2_SQLLOG 
	    ...
	    WHERE  ([Query Details] like '%' + @FIELDNAME + '%') AND [Query Details] like '%' + @TBNAME + '%'
	...
	SELECT @LASTREFRESHDATA =[LastRunDateTime] FROM [ANALYSIS_LOG].[dbo].[M_BI_LOG_DW2_EXECUTION] WHERE [PackageName]='RDBUserTraceLog'
...
--exec [SP_SqlDataRefresh1]
--SELECT * FROM T_BI_LOG_DW2_SQLLOG_STAGING
GO
